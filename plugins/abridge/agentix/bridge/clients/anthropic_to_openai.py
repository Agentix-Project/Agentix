...
from ..proxy import ClientResponse, Handler, Request, TunnelHandle, on
...
class AnthropicToOpenAI:
    ...
    This layer does not own the downstream's lifecycle: hold the forwarder yourself
    to read `.session_id` / call `delete_session()` / `aclose()`.
    """

    def __init__(self, downstream: Handler, *, model: str | None = None) -> None:
        self._downstream = downstream
        self._model = model
        # Per-tool-call-id memory of the EXACT assistant message the downstream
        # returned. The Anthropic round-trip is lossy (it drops reasoning_content
        # and the per-tool-call `index`), so a reconstructed assistant won't match
        # what a session-recording backend (TITO) stored byte-for-byte. On the next
        # turn we replay the remembered original verbatim, keyed by the tool-call
        # ids that survive the round-trip — making the history identical from the
        # backend's POV and immune to whatever private fields the backend keeps.
        self._assistant_by_ids: dict[tuple[str, ...], dict[str, Any]] = {}
    ...
    @staticmethod
    def _ids(message: dict[str, Any]) -> tuple[str, ...]:
        return tuple(
            tc["id"]
    ...
    def _remember_assistant(self, openai_resp: dict[str, Any]) -> None:
        ...
        ids = self._ids(message)
        if ids:
            self._assistant_by_ids[ids] = message
    ...
    def _replay_remembered_assistants(self, openai_body: dict[str, Any]) -> None:
        ...
        for i, msg in enumerate(messages):
            if not isinstance(msg, dict) or msg.get("role") != "assistant":
                continue
            remembered = self._assistant_by_ids.get(self._ids(msg))
