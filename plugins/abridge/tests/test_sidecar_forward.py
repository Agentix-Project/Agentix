...
async def test_abridge_error_status_survives_tunnel_and_sio(monkeypatch) -> None:
    """An `AbridgeError(429)` raised by a host handler reaches the agent's HTTP
    response as a 429 through the real `:error` reply path — the sandbox must
    not collapse it to a blanket 502."""
    import agentix.bridge.proxy as proxy_mod
    ...
    class _RateLimited:
        ...
        async def handle(self, request: Request) -> ClientResponse:
            raise AbridgeError("rate limited upstream", status_code=429)

    host = Proxy(_RateLimited())
    handle = await proxy_mod._start_tunnel(paths=list(host.paths))
    sandbox_ns = proxy_mod._get_namespace()
    ...
    try:
        ...
        assert response.json()["error"]["message"] == "rate limited upstream"
    finally:
        await proxy_mod._stop_tunnel(handle=handle)
...
async def test_proxy_session_teardown_error_surfaces_inside_except_block() -> None:
    """A teardown failure after a SUCCESSFUL body must raise even when the
    caller sits inside an `except` block (a common retry/fallback shape) —
    ambient exception state must not be mistaken for a body failure."""

    class _ExplodingClose:
        def abridge_routes(self):
            return {"/x": self.handle}
    ...
    proxy = Proxy(_ExplodingClose())
    try:
        raise ValueError("ambient exception being handled")
    except ValueError:
        with pytest.raises(ExceptionGroup):
            async with proxy.session(_FakeSandbox()):
                pass
...
async def test_session_forward_accepts_any_2xx_create(monkeypatch) -> None:
    """A gateway that answers the create POST with 201 (or any 2xx) is a
    success, not a 502."""
    fwd = SessionForward("http://gw", paths=["/v1/chat/completions"])

    async def fake_post(url, *, json, headers):
        return httpx.Response(201, content=b'{"session_id": "S201"}', headers={"content-type": "application/json"})

    assert fwd._client is not None
    monkeypatch.setattr(fwd._client, "post", fake_post)
    assert await fwd.open() == "S201"


async def test_session_forward_attaches_to_assigned_session(monkeypatch) -> None:
    """Assigning `session_id` attaches to that existing gateway session:
    forwarded requests are scoped to it with NO create call — the assigned id
    must not be silently overwritten by a fresh create."""
    fwd = SessionForward("http://gw", paths=["/v1/chat/completions"])
    fwd.session_id = "EXT"
    calls: list = []
    ...
    assert fwd.session_id == "EXT"
    # exactly one upstream call — straight into the attached session, no create.
    assert [url for url, _ in calls] == ["http://gw/sessions/EXT/v1/chat/completions"]
    assert calls[0][1]["x-session-id"] == "EXT"
...
async def test_delete_session_after_close_does_not_resurrect_pool(monkeypatch) -> None:
    """The documented harvest flow — Proxy.stop (pool closed), harvest, then
    `delete_session()` — must not leave behind a lazily recreated pool that no
    remaining lifecycle will ever close."""
    fwd = SessionForward("http://gw", paths=["/v1/chat/completions"])
    deleted: list = []
...
async def test_proxy_stop_closes_forwarder_pool_through_converter() -> None:
    """`Proxy(AnthropicToOpenAI(tito.handler()))` — the converter must forward
    `aclose()` to its downstream's owner, or the SessionForward httpx pool
    outlives `Proxy.stop`. The session itself is untouched (harvest survives)."""
    from agentix.bridge.clients import AnthropicToOpenAI
    ...
    proxy = Proxy(AnthropicToOpenAI(tito.handler(), model="m"))
    async with proxy.session(_FakeSandbox()):
...
async def test_anthropic_to_openai_replay_survives_reused_tool_call_ids() -> None:
    """Some OpenAI-compatible servers reuse tool-call ids across turns (TGI
    famously emits id "0"). The replay memory must not key on ids alone, or a
    later turn overwrites an earlier one and every matching assistant in the
    resent history is replaced by the LAST remembered message — silently
    corrupting the recorded conversation."""
    from agentix.bridge.clients import AnthropicToOpenAI

    def _assistant(args: str, reasoning: str) -> dict:
        return {
            "role": "assistant",
            "content": "",
            "reasoning_content": reasoning,
            "tool_calls": [{
                "id": "0", "index": 0, "type": "function",
                "function": {"name": "python", "arguments": args},
            }],
        }

    # Distinct calls, same id "0"; arguments spacing differs from the
    # round-trip's json.dumps rendering to prove key canonicalization.
    assistant_1 = _assistant('{"expression":"1+1"}', "r1")
    assistant_2 = _assistant('{"expression":"2+2"}', "r2")
    replies = [assistant_1, assistant_2, _assistant('{"expression":"3+3"}', "r3")]
    sent: list = []
    ...
    async def fake_downstream(request: Request) -> ClientResponse:
        ...
        return ClientResponse.json({
            "choices": [{"index": 0, "finish_reason": "tool_calls",
                         "message": replies[len(sent) - 1]}],
            "model": "m", "usage": {},
        })

    def _tool_use(args: dict) -> dict:
        return {"role": "assistant", "content": [
            {"type": "tool_use", "id": "0", "name": "python", "input": args}]}

    def _tool_result(text: str) -> dict:
        return {"role": "user", "content": [
            {"type": "tool_result", "tool_use_id": "0", "content": text}]}

    conv = AnthropicToOpenAI(fake_downstream, model="m")
    turn = [{"role": "user", "content": "hi"}]
    await conv.messages(_req("/v1/messages", {"model": "c", "max_tokens": 64, "messages": turn}))
    turn = [*turn, _tool_use({"expression": "1+1"}), _tool_result("2")]
    await conv.messages(_req("/v1/messages", {"model": "c", "max_tokens": 64, "messages": turn}))
    turn = [*turn, _tool_use({"expression": "2+2"}), _tool_result("4")]
    await conv.messages(_req("/v1/messages", {"model": "c", "max_tokens": 64, "messages": turn}))

    asst = [m for m in sent[2] if m.get("role") == "assistant"]
    assert len(asst) == 2
    # Each history slot replays ITS OWN remembered message, not the latest one.
    assert asst[0] == assistant_1
    assert asst[1] == assistant_2
