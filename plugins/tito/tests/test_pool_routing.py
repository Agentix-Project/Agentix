...
@pytest.mark.asyncio
async def test_successful_proxy_reports_backend_up(monkeypatch):
    """A backend that answers a proxied request is healthy — the success path
    must clear a stale down mark (the only other recovery is the cooldown)."""
    pool = BackendPool([A], policy="sticky")
    pool.report_down(A)  # sole backend: the all-down fallback still picks it
    srv = SessionServer(_args(), pool)

    async def ok(method, url, content=None, headers=None):
        return _Resp()

    monkeypatch.setattr(srv._backend.client, "request", ok)
    await srv._backend.do_proxy(_Request("/sessions/s1/v1/chat/completions"), "v1/chat/completions")
    assert not pool._down  # noqa: SLF001
