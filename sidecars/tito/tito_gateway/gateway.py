...
from tito_gateway.discovery import discover_backend_url, normalize_backend_url
...
class TITOGateway:
    ...
    def __init__(self, config: TITOGatewayConfig):
        ...
        self.server = SessionServer(self.config.as_miles_namespace(), self.pool)
    ...
    def _register_health_alias(self) -> None:
        # abridge's Sidecar probes `/healthz` by default; the vendored session
        # server only exposes `/health`. Add a thin alias so a default Sidecar
