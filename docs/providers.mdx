...
```python title="my_provider/__init__.py"
import os
...
The `cape` backend targets a lease-based GPU capacity pool driven by a
...
[`plugins/providers/cape/README.md`](https://github.com/Agentix-Project/Agentix/tree/master/plugins/providers/cape).
...
| `CAPE_CONTROLLER_URL` / `CAPE_TOKEN` / `CAPE_TOKEN_FILE` / `CAPE_BINARY` | `cape` backend | Controller endpoint, authentication, and CLI selection |

The `daytona` and `e2b` backends are placeholders today: they validate
...
[`plugins/providers/uv/README.md`](https://github.com/Agentix-Project/Agentix/tree/master/plugins/providers/uv)).
