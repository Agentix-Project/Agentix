...
3. Loads the resulting bundle image into Docker and also applies a
...
`agentix build` defaults to the portable tar bundle format. Use
`--format oci-image` when you want to run the bundle with
`DockerDeployment`.
