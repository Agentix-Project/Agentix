...
Remote calls use Socket.IO events on the `/rpc` namespace — one
transport for every call, short or long-running. HTTP serves only the
`/health` probe.
...
| `/log` | sandbox → host | captured stdout/stderr lines, best-effort (auto-registered) |
