...
`client.remote(fn, ...)` returns the **unwrapped** result of `fn` — type
...
n: int = await client.remote(total, 1, 2)        # R = int
data: bytes = await client.remote(fetch, "...")  # R = bytes, not Awaitable[bytes]
```
