...
The client serializes a callable reference from the function object:

```python
callable_ref = f"{run.__module__}::{run.__qualname__}"
```

Arguments are serialized separately with stdlib pickle:

```python
arguments = pickle.dumps((args, kwargs))
```

The runtime worker resolves the callable reference inside the sandbox,
unpickles arguments, validates them, and invokes the target.
