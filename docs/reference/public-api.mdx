...
`RuntimeClient(url, timeout=1800)`. Every call rides one transport —
Socket.IO on `/rpc`. To bound a single call independently of the request
timeout, wrap it in `asyncio.wait_for(sandbox.remote(...), deadline)`.
...
- `agentix plugin list` — read-only listing of installed provider backends
