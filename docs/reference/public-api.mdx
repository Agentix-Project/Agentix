...
Failure and result vocabulary (raised by / returned from `.remote` and
`.try_remote`):
...
- `RuntimeUnreachable`
- `RemoteSioError`
- `RestrictedUnpickleError` (a refused return-value global — see
  [Remote Calls](/concepts/remote-calls))
...
The worker also captures the process's raw stdout and stderr (`print()`,
